"""Unified task/mode CLI.
    ...
    ptx <mode> [args...]              # task 由配置决定(推荐)
    ptx <task> <mode> [args...]       # 显式写 task(等价 + 一致性校验)
    python -m pytorchx <mode> ...     # 免安装等价写法

    ptx train   -c configs/yolo/yolov8_graph.yml -o Global.epoch_num=100
    ptx val     -c configs/yolo/yolov8-obb_graph.yml --weights output/x/best_accuracy.pth
    ptx check   -c configs/attr/vehicle_attribute.yml
    ptx export  -c configs/yolo/yolov8-pose_graph.yml --weights ... --onnx
    ptx predict -c configs/yolo/yolov8-pose_graph.yml --weights ... --input imgs
    ptx data list | ptx data get <name>          # 数据集(ModelScope)下载/检查
  ptx detect train -c configs/yolo/yolov8_graph.yml        # 显式 task
...
USAGE = """用法: ptx [task] <mode> [args...]        (task 可省略,由配置 Architecture.task 决定)
...
显式写 task(等价,并做一致性校验):
  ptx detect train -c configs/yolo/yolov8_graph.yml
...
def main(argv=None):
    ...
    if argv[0].lower() in ("data", "dataset", "datasets"):
        from pytorchx.datasets import data_main
    ...
    mod = _load_script(SCRIPT[mode], "ptx_" + mode)
