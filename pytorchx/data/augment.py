...
class RandomAffine(object):
    ...
    def _matrix(self, w, h):
        ...
        if self.degrees or self.scale != 1.0:
            M = cv2.getRotationMatrix2D(
                (W / 2, H / 2), _rand(-self.degrees, self.degrees),
                _rand(1 - self.scale, 1 + self.scale),
            )
            dst = cv2.transform(dst.reshape(1, -1, 2), M).reshape(-1, 2)
        if self.shear:
            sh_t = np.tan(np.deg2rad(_rand(-self.shear, self.shear)))
            S = np.array([[1, sh_t, 0], [0, 1, 0], [0, 0, 1]], np.float32)
            dst = _warp_points(S, dst)
        if self.translate:
            dst[:, 0] += _rand(-self.translate, self.translate) * W
            dst[:, 1] += _rand(-self.translate, self.translate) * H
        if self.perspective:
            j = self.perspective * max(W, H)
            dst = dst + np.random.uniform(-1, 1, dst.shape).astype(np.float32) * j
        # 源 = 输入图像四角(输入可能是原图,也可能是 mosaic 大画布)
        src = np.array(
            [[0, 0], [w, 0], [w, h], [0, h]], np.float32
        )
        return cv2.getPerspectiveTransform(src, dst), s
...
def _random_crop_mosaic(canvas, labels, kpts, masks, S, box_format, center=None):
    """把 2S/3S 的 mosaic 画布随机裁出 ``S`` 窗口(对齐 ultralytics:对象保持 ~1x)。

    原版 ``RandomPerspective`` 不缩放画布,而是把大画布裁剪成 imgsz;若不裁剪而直接
    缩放,对象会被缩小一倍(实测面积 0.0044 vs 原版 0.0107)。
    ``center`` 指定裁窗中心(mosaic 的 tile 汇聚点),保证窗口覆盖四个 patch 的核心对象。
    """
    h, w = canvas.shape[:2]
...
def mosaic4(items, imgsz, box_format="xyxy"):
    ...
    canvas, labels, kpts, masks = _random_crop_mosaic(
        canvas, labels, kpts, masks, s, box_format, center=(xc, yc)
    )
    return canvas, labels, kpts, masks
