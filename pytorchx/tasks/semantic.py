...
class YoloSemTask(TaskAdapter):
    ...
    def build_post_process(self, config):
        from pytorchx.sem import build_sem_postprocess
    ...
    def build_loss(self, config, model):
        from pytorchx.sem import build_sem_loss

        return build_sem_loss(config.get("Loss"))

    def build_metric(self, config):
        from pytorchx.sem import build_sem_metric
    ...
    def build_datasets(self, config, logger):
        from pytorchx.data.sem import SemDataset
    ...
    def train_collate(self, batch):
        from pytorchx.data.sem import train_collate

        return train_collate(batch)

    def eval_collate(self, batch):
        from pytorchx.data.sem import eval_collate
