...
# The upstream PaddleOCR implementation of IaaAugment relies on albumentations.
# These imports are kept guarded (analogous to guarding imgaug) so that merely
# importing this module does not fail when albumentations / packaging are not
# installed.  The numerical behaviour is unchanged when they are available.
try:
