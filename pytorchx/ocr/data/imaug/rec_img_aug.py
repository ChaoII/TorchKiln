...
#
#  The original module relies on ``paddle.get_device()`` and
#  ``paddle.vision.transforms.Compose``.  Neither is available in a
#  PyTorch environment and the numerical behaviour of the operators below
#  does not depend on them.  ``get_device`` keeps the exact same
#  conditional semantics (the ``"iluvatar_gpu"`` branch is only taken on
#  Iluvatar hardware), while ``Compose`` reproduces Paddle's trivial
#  sequential-transform callable.
# ------------------------------------------------------------------ #
def get_device():
    """Return the current device name.

    In a PyTorch environment there is no Iluvatar GPU device string, so the
    ``"iluvatar_gpu" in get_device()`` checks scattered through the resize
    operators evaluate to ``False`` exactly as they do on ordinary
    Paddle CPU/CUDA builds.
    """
    return ""


class Compose(object):
    """Apply a list of transforms sequentially (Paddle-compatible)."""
...
# ------------------------------------------------------------------ #
#  Text-Image-Augmentation helpers (ported from
#  ppocr/data/imaug/text_image_aug/{warp_mls.py,augment.py}).
#  Inlined here so the rec pipeline is self-contained and does not
#  require the optional text_image_aug package.
# ------------------------------------------------------------------ #
class WarpMLS:
...
class ABINetRecAug(object):
    def __init__(
        ...
        # ``abinet_aug`` is optional (it depends on Paddle vision transforms),
        # so it is imported lazily to keep module import safe when absent.
...
class SVTRRecAug(object):
    def __init__(
        ...
        **kwargs,
        ...
        # Optional dependency, imported lazily (see ABINetRecAug).
