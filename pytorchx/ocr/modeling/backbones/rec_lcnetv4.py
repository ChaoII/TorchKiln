...
class Conv2D_BN(nn.Sequential):
    ...
    def fuse(self):
        ...
        # Keep 'same' padding as-is (asymmetric for even kernels), otherwise
        # the fused conv would change the output spatial size.
        padding = c.padding
        m = nn.Conv2d(
            w.shape[1] * c.groups,
            w.shape[0],
...
class ConvBNAct(nn.Module):
    ...
    def rep(self):
        ...
        # Keep 'same' padding as-is. For even kernels (e.g. the k=2 stem convs)
        # 'same' means asymmetric padding (0 on top/left, 1 on bottom/right);
        # converting it to (k-1)//2 would shrink the output by 1 each time and
        # break the later concat in StemBlock.
        padding = c.padding
        m = nn.Conv2d(
            c.in_channels,
            c.out_channels,
