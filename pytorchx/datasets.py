...
仓库只入库 label 文本;图片打包在 ModelScope(见 ``datasets/manifest.yml``)。
    ...
    ptx data list                 # 查看每个数据集:URL / label / 图片 是否就绪
    ptx data get plate_det_demo   # 下载 + 解包到 datasets/plate_det_demo/
    ptx data get --all            # 逐个下载(未填 URL 的会提示)
    ptx data verify               # 只做就绪性检查(等价于 list)
...
def list_datasets(path=None):
    ...
    if todo:
        ...
        print("下载:ptx data get <name>(或让 tools/make_demo_data.py 生成本地占位图)")
    return 0
...
def _unpack(archive, target):
    tmp = tempfile.mkdtemp(prefix="ptx_data_")
    try:
...
def get_dataset(name, path=None, force=False, root=None):
    ...
    with tempfile.TemporaryDirectory(prefix="ptx_dl_") as td:
