...
def build_arch_model(arch, task=None):
    ...
    * otherwise -> the hand-written per-task builder
    ...
    if arch.get("yaml_file") or arch.get("yaml_text"):
        from pytorchx.nn.graph import build_from_arch
    ...
    if task in ("detect", "obb"):
        from pytorchx.models.det import build_model as _m

        return _m(arch)
    if task == "segment":
        from pytorchx.models.seg import build_model as _m

        return _m(arch)
    if task == "semantic":
        from pytorchx.models.sem import build_model as _m

        return _m(arch)
    if task == "depth":
        from pytorchx.models.depth import build_model as _m

        return _m(arch)
    if task == "plate_rec":
        from pytorchx.models.plate import build_rec_model

        return build_rec_model(arch)
    if task == "attribute":
        from pytorchx.attr import build_attribute_model
    ...
    if task == "plate_det":
        ...
        arch = dict(arch)
        arch.setdefault("yaml_file", "pytorchx/cfg/models/plate/yolov5n-0.5.yaml")
        return build_from_arch(arch)
