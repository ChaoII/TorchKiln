...
from torchkiln.det.metric import DetMetric, _ap_101
...
def _semantic_loss(pred_semantic, batch, num_classes):
    """YOLO26 语义辅助损失(BCE+Dice),对齐 ultralytics ``BCEDiceLoss`` + ``v8SegmentationLoss``。

    ``pred_semantic``: ``(B, nc, h, w)``;target 由 GT 实例掩码 + 类别 id 构建:覆盖像素处
    该类通道置 1,未覆盖像素全 0;target 下采样到 pred 分辨率(nearest)。
    """
    device = pred_semantic.device
    b, nc, h, w = pred_semantic.shape
    gt_all = batch[3].to(device).float()  # (B, N, H, W) stride-4 实例掩码
    labels = batch[1][..., 0].to(device).long()  # (B, N)
    cls_map = (gt_all * labels[:, :, None, None].float()).max(1).values  # (B, H, W)
    present = (gt_all.sum(1) > 0).float()  # (B, H, W)
    sem_t = torch.zeros(b, nc, *present.shape[-2:], device=device, dtype=pred_semantic.dtype)
    sem_t.scatter_(1, cls_map.long().unsqueeze(1), present.unsqueeze(1))
    if tuple(sem_t.shape[-2:]) != (h, w):
        sem_t = F.interpolate(sem_t, (h, w), mode="nearest")
    bce = F.binary_cross_entropy_with_logits(pred_semantic, sem_t)
    p = pred_semantic.sigmoid()
    inter = (p * sem_t).sum(dim=(2, 3))
    union = p.sum(dim=(2, 3)) + sem_t.sum(dim=(2, 3))
    dice_loss = (1.0 - (2.0 * inter + 1.0) / (union + 1.0)).mean()
...
class SegLoss(DetLoss):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            box_gain=box_gain,
            reg_max=reg_max,
            use_one2one=use_one2one,
            one2one_topk=one2one_topk,
            one2one_topk2=one2one_topk2,
            e2e_gain_schedule=e2e_gain_schedule,
            e2e_o2m=e2e_o2m,
            e2e_final_o2m=e2e_final_o2m,
    ...
    def forward(self, preds, batch):
        if (
            ...
            one2many, one2one, proto = preds
            d1 = self._forward_one(one2many, proto, batch, self.assigner)
            # 对齐 ultralytics:one2one 分支的 proto(含语义)需 detach(不回流骨干)
            proto_o2o = (
                tuple(p.detach() for p in proto)
                if isinstance(proto, (tuple, list))
                else proto.detach()
            )
            d2 = self._forward_one(one2one, proto_o2o, batch, self.assigner_one2one)
            g1, g2 = self._e2e_gains()
            out = {"loss": g1 * d1["loss"] + g2 * d2["loss"]}
            for key in d1:
                if key != "loss":
                    out[key] = d1[key]
            for key in d2:
        ...
        feats = preds["feats"]
        protos = preds["protos"]
        out = self._forward_one(feats, protos, batch, self.assigner)
        out["loss"] = out["loss"] * int(batch[0].shape[0])
        return out

    def _forward_one(self, feats, protos, batch, assigner):
        pred_dist_raw, pred_scores, pred_coeff = self._split_full(feats)
        if isinstance(protos, (tuple, list)):
            protos, pred_semantic = protos[0], protos[1]
        else:
            pred_semantic = None
        pred_distri = (
        ...
        with torch.no_grad():
            t_labels, t_bboxes, t_scores, fg, t_gt_idx = assigner(
        ...
        if bool(fg.any()):
            ...
            if self.reg_max > 1:
                ...
                # L1 loss for reg_max<=1 (YOLO26), matching ultralytics BboxLoss l1 branch
                b_sz, a_sz = fg.shape
                stride_fg = stride_tensor.squeeze(-1).unsqueeze(0).expand_as(fg)[fg]
                ap_fg = anchor_points.unsqueeze(0).expand(fg.shape[0], -1, -1)[fg]
                tgt = t_bboxes[fg] / stride_fg[:, None]
        ...
        if bool(fg.any()) and batch[3] is not None:
            ...
            # 对齐 ultralytics overlap_mask=True:数据集是实例索引图(后画实例覆盖先画),
            # 实例 k 的 GT 掩码 = 索引图 == k+1(重叠像素归后画实例)。这里用逐实例掩码
            # 重建索引图(max 取较大实例号),与 ultra 语义一致(重叠像素仅占 ~2%)。
            _n_inst = _gt_all.shape[1]
            _ids = torch.arange(
                1, _n_inst + 1, device=device, dtype=torch.float32
            ).view(1, _n_inst, 1, 1)
            _idx_map = (_gt_all.float() * _ids).amax(1)  # (B, h, w)
            loss_mask = 0.0
            for b in range(protos.shape[0]):
                ...
                if not bool(sel.any()):
                    continue
                gidx = t_gt_idx[b][sel]
                gm = (_idx_map[b].unsqueeze(0) == (gidx + 1).view(-1, 1, 1)).float()
        ...
        # ---- semantic auxiliary loss (yolo26 seg, aligns ultralytics v8SegmentationLoss) ----
        if pred_semantic is not None and batch[3] is not None:
            loss_sem = _semantic_loss(pred_semantic, batch, self.num_classes)
        else:
            loss_sem = pred_coeff.sum() * 0.0
...
def build_seg_loss(loss_cfg, num_classes, nm=None, reg_max=None, epochs=None):
    ...
    if reg_max is not None and "reg_max" not in cfg:
        cfg["reg_max"] = reg_max
    if epochs and "e2e_epochs" not in cfg:
        cfg["e2e_epochs"] = int(epochs)
