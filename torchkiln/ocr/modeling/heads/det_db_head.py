...
from torchkiln.ocr.modeling.common import Activation
from torchkiln.ocr.modeling.backbones.det_mobilenet_v3 import ConvBNLayer
...
class DBHead(nn.Module):
    ...
    def __init__(self, in_channels, k=50, aux_in_channels=0, **kwargs):
        ...
        if aux_in_channels > 0:
            self._aux_upsample_scale = {"aux_p4": 4, "aux_p3": 2, "aux_p2": 1}
            for lvl in ("p4", "p3", "p2"):
                setattr(self, "aux_binarize_" + lvl, Head(aux_in_channels, **kwargs))
                setattr(self, "aux_thresh_" + lvl, Head(aux_in_channels, **kwargs))
