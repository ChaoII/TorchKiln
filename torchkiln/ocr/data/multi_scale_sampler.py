...
class MultiScaleSampler(torch.utils.data.Sampler):
    """Multi-scale batch sampler (torch port of PaddleOCR's sampler).

    It is a ``torch.utils.data.BatchSampler``-like iterable: ``__iter__``
    yields one ``list`` of dataset indices per batch, while the per-batch
    resolution / wh-ratio metadata computed by Paddle's version is preserved
    in ``self.batch_list`` and ``self.batchs_in_one_epoch``.

    Args:
        data_source(dataset): dataset exposing ``data_idx_order_list`` (and,
            when ``ds_width`` is enabled, ``wh_ratio`` / ``wh_ratio_sort``).
        scales(list): several scales for image resolution.
        first_bs(int): batch size for the first scale in scales.
        fix_bs(bool): keep the first batch size for every scale.
        divided_factor(list[w, h]): ImageNet models down-sample images by a
            factor, ensure that width and height dimensions are multiples of
            ``divided_factor``.
        is_training(boolean): mode.
        ratio_wh(float): reserved for API compatibility.
        max_w(float): maximum image width used for the width-aware ratio.
        seed(int|None): seed used to shuffle the batches each epoch.
    """
    ...
    def __init__(
        ...
        self.ds_width = getattr(data_source, "ds_width", False)
        self.seed = getattr(data_source, "seed", None)
        if self.ds_width:
        ...
        if is_training:
            ...
            # Ensure that width and height dimensions are multiples of 32.
            width_dims = [
    ...
    def __iter__(self):
        ...
        for batch_tuple_id in self.batchs_in_one_epoch_id:
            batch = self.batchs_in_one_epoch[batch_tuple_id]
            # Yield the dataset indices for this batch (BatchSampler contract).
            yield [properties[2] for properties in batch]
