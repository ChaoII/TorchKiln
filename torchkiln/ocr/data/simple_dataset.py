...
class SimpleDataSet(Dataset):
    def __init__(self, config, mode, logger=None, seed=None):
        super(SimpleDataSet, self).__init__()
        if logger is None:
            import logging

            logger = logging.getLogger("torchkiln/ocr")
        self.logger = logger
        self.mode = mode.lower()

        global_config = config.get("Global", {})

        # `config` may either be the full configuration (with a mode section
        # such as "Train"/"Eval") or the mode sub-dict itself (containing
        # "dataset" and "loader").  Resolve the dataset/loader section for the
        # requested mode, mirroring PaddleOCR's `config[mode]` lookup.
        mode_config = None
        for key in config.keys():
            if (
                isinstance(key, str)
                and key.lower() == self.mode
                and isinstance(config[key], dict)
                and "dataset" in config[key]
            ):
                mode_config = config[key]
                break
        if mode_config is None:
            mode_config = config

        dataset_config = mode_config["dataset"]
        loader_config = mode_config["loader"]
...
def det_collate_fn(batch):
    """Collate function for text-detection.

    Empty/None samples are discarded (PaddleOCR's det collate drops them) and
    the remaining per-key lists of numpy arrays are stacked into
    ``torch.float32`` tensors along a new batch dimension.
    """
    batch = [sample for sample in batch if sample is not None and len(sample) > 0]
    if len(batch) == 0:
        return []
...
def build_dataset(config, mode, logger=None, seed=None):
    """Build a :class:`SimpleDataSet` for the given ``mode``.

    ``config`` is expected to be the ``Train``/``Eval`` sub-dict containing the
    ``dataset`` and ``loader`` sections.  For convenience the full configuration
    dict (with the mode section nested inside) is accepted as well.
    """
    mode = mode.lower()
    if isinstance(config, dict) and "dataset" in config and "loader" in config:
        # `config` is the Train/Eval sub-dict itself.
        full_config = {"Global": config.get("Global", {}), mode: config}
    else:
        full_config = config
    return SimpleDataSet(full_config, mode, logger, seed)
