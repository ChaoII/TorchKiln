...
class CTCLoss(nn.Module):
    ...
    def forward(self, predicts, batch):
        ...
        # Paddle's nn.CTCLoss applies log_softmax internally; PyTorch expects
        # log-probabilities, so apply it here for numerical parity.
        predicts = F.log_softmax(predicts, dim=2)
        N, B, _ = predicts.shape
        preds_lengths = torch.tensor(
            [N] * B, dtype=torch.int64, device=predicts.device
        )
        ...
        if self.use_focal_loss:
            ...
            weight = torch.subtract(
                torch.tensor([1.0], device=predicts.device), weight
            )
