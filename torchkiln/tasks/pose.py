...
class YoloPoseTask(TaskAdapter):
    ...
    def build_post_process(self, config):
        from torchkiln.pose import build_pose_postprocess
    ...
    def build_loss(self, config, model):
        from torchkiln.pose import build_pose_loss
    ...
    def build_metric(self, config):
        from torchkiln.pose import build_pose_metric

        return build_pose_metric(config.get("Metric"))

    def build_datasets(self, config, logger):
        from torchkiln.data.pose import PoseDataset
    ...
    def train_collate(self, batch):
        from torchkiln.data.pose import train_collate

        return train_collate(batch)

    def eval_collate(self, batch):
        from torchkiln.data.pose import eval_collate
