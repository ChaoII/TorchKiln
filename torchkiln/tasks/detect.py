...
from torchkiln.tasks._base import _class_weights_from_config  # noqa: F401
...
class YoloDetTask(TaskAdapter):
    ...
    def build_post_process(self, config):
        from torchkiln.det import build_det_postprocess
    ...
    def build_loss(self, config, model):
        from torchkiln.det import build_det_loss
    ...
    def build_metric(self, config):
        from torchkiln.det import build_det_metric

        return build_det_metric(config.get("Metric"))

    def build_datasets(self, config, logger):
        from torchkiln.data.det import DetDataset
    ...
    def train_collate(self, batch):
        from torchkiln.data.det import train_collate

        return train_collate(batch)

    def eval_collate(self, batch):
        from torchkiln.data.det import eval_collate
