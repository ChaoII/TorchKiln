...
class DetMetric(object):
    ...
    def _iou(self, box, boxes):
        ...
        if self.box_format == "xywhr":
            from torchkiln.det.rbox import probiou
    ...
    def _iou_mat(self, pb, gt):
        """(M,4/5) vs (N,4/5) -> (M,N) IoU, vectorized (一次性算完)."""
        if pb.shape[0] == 0 or gt.shape[0] == 0:
            return np.zeros((pb.shape[0], gt.shape[0]), dtype=np.float32)
        if self.box_format == "xywhr":
            from torchkiln.det.rbox import probiou
    ...
    def get_metric(self):
        ...
        # 预计算每个 (图,类) 的 IoU 矩阵**一次**,各 IoU 阈值间复用(原来每个阈值都
        # 重算逐框 IoU,10x 冗余且是 Python 逐框调用 -> 评估极慢)。
        cache = []
        for i in range(num_images):
            ...
            for c in range(n_classes):
                ...
                per_c[c] = (ps, self._iou_mat(pb, gt_c), gt_c.shape[0])
            cache.append(per_c)
        ...
        for thr in self.iou_thresholds:
            ...
            for c in range(n_classes):
                ...
                for i in range(num_images):
                    ps, iou, ng = cache[i][c]
                    n_gt += ng
                    matched = np.zeros(ng, dtype=bool)
                    for j in range(ps.shape[0]):
                        ...
                        if ng:
                            ...
                            row = iou[j] if iou.shape[1] else np.zeros(0, np.float32)
                            k = -1
