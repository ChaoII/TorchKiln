...
from torchkiln.det.rbox import dist2rbox, points_in_rboxes, probiou, rbox2dist
...
class DetLoss(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # ultralytics E2ELoss 的 one2many/one2one 增益调度(o2m: init→final,o2o=1-o2m)
        self.e2e_gain_schedule = bool(e2e_gain_schedule)
        self._o2m = float(e2e_o2m)
        self._o2o = max(1.0 - self._o2m, 0.0)
        self._o2m_copy = float(e2e_o2m)
        self._final_o2m = float(e2e_final_o2m)
        self._e2e_epochs = int(e2e_epochs) if e2e_epochs else 0
        self._updates = 0
        if self.use_one2one:
    ...
    def _e2e_gains(self):
        """one2many/one2one 增益:调度开启用 o2m/o2o,否则固定 (1.0, one2one_gain)。"""
        if self.e2e_gain_schedule:
            return self._o2m, self._o2o
        return 1.0, self.one2one_gain

    def update(self):
        """每个 epoch 末调用,按 ultralytics E2ELoss 线性衰减 one2many 增益。"""
        if not (self.use_one2one and self.e2e_gain_schedule and self._e2e_epochs > 0):
            return
        self._updates += 1
        self._o2m = max(
            1 - self._updates / max(self._e2e_epochs - 1, 1), 0
        ) * (self._o2m_copy - self._final_o2m) + self._final_o2m
        self._o2o = max(1.0 - self._o2m, 0.0)
